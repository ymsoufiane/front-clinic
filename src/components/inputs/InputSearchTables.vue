<template>
    <div class="flex justify-between py-4">
        <div class="flex items-center">
            ...
            <InputSelect class="py-0" @changeValue="setPageSize" :input="inputSelect" />
        </div>
        <div>
            <div class="grid grid-cols-3 text-sm text-[#333] font-normal">
                <div @click="exportData(item)" v-for="item, index in exportsItems" :key="index"
                    class="border-[#e4e4f3] border-2 p-2 cursor-pointer">{{ item['name'] }}</div>
            </div>
        </div>
        <div class="flex">
            <div>
